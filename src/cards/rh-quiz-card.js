class RHQuizCard extends HTMLElement {
  ...
  _buildRenderKey() {
    ...
    const roundKey = roundState
      ? `${roundState.state}:${roundState.attributes?.active_round_index ?? ""}:${roundState.attributes?.active_round_name ?? ""}:${roundState.attributes?.round_position_index ?? ""}:${Array.isArray(roundState.attributes?.quiz_rounds) ? roundState.attributes.quiz_rounds.join("|") : ""}:${roundState.attributes?.total_rounds ?? ""}`
      : "";
    return [
      roundKey,
  ...
  _roundInfoSection(fullBleed = false) {
    ...
    const roundCounter = hasActiveRound
      ? (activeIndex !== null && totalRounds > 0 ? `Round ${activeIndex + 1} of ${totalRounds}` : "")
  ...
  _render() {
    ...
    const contentStyle = fullBleedRoundInfo
